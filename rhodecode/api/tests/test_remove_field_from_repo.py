...
class TestRemoveFieldFromRepo(object):
    def test_api_remove_field_from_repo(self, backend):
        ...
        id_, params = build_data(
            self.apikey, 'remove_field_from_repo',
            repoid=repo_name,
            key='extra_field')
        response = api_call(self.app, params)
        expected = {
            'msg': 'Deleted repository field `extra_field`',
            'success': True,
        }
        assert_ok(id_, expected, given=response.body)
        repo = Repository.get_by_repo_name(repo_name)
